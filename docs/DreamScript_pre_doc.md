...
+ 关于 `阻塞节点` 和 `非阻塞节点`：到目前为止，除去 `绑定指令` 和 `标签` 为非阻塞节点之外，其余有效节点（即除注释之外的节点）均为阻塞节点，二者唯一的区别便是在一次推进（推进的定义见指令部分介绍）的过程中，阻塞节点会在执行完本身后让推进过程停下来，直到激活下一次推进，而非阻塞的节点则会执行完本身后继续向下执行；所以，多个绑定指令会在一次推进过程中逐条执行，如果想停止此过程，使用 `独立指令` 是一个好主意
